...
function populateAlphabetSelector() {
  const select = document.getElementById('alphabet-lang-select');
  if (!select) return;
  select.innerHTML = '<option value="">Sélectionner une langue...</option>';
  coreLanguages.forEach(lang => {
    const opt = document.createElement('option');
    opt.value = lang.id;
    opt.textContent = `${languageFlags[lang.id] || '🌍'} ${lang.name} (${lang.iso})`;
    select.appendChild(opt);
  });
}

function modalOpenAlphabet() {
  hideLangModal();
  const select = document.getElementById('alphabet-lang-select');
  if (select && currentLangId) {
    select.value = currentLangId;
    performAlphabetAnalysis(currentLangId);
  }
